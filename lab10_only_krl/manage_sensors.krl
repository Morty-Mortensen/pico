ruleset manage_sensors {
    ...
    meta {
      use module manage_sensors_profile alias profile
        ...
        provides showChildren, sensors, sensorECIByName, getTemperaturesOfAllChildren, sensorBySubscription, getEci, getNumSenesors, getFiveLatestReports, reports
        shares showChildren, sensors, sensorECIByName, getTemperaturesOfAllChildren, sensorBySubscription, getEci, getNumSenesors, getFiveLatestReports, reports
    }
    ...
    global {
        ...
        getFiveLatestReports = function() {
          reportKeys = reports().keys()
          endIndex = (reportKeys.length() >= 5) => 4 | (reportKeys.length() == 0) => reportKeys.length() | reportKeys.length() - 1
          updatedKeys = reportKeys.reverse().slice(0, endIndex)
          filteredReports = reports().filter(function(v,k) {updatedKeys.any(function(x) {x == k})})
          filteredReports
        }
    ...
    rule sensors_begin_report {
      select when section begin_report

      pre {
        report_id = random:uuid().klog("REPORT ID: ")
      }
    ...
    rule sensors_report {
      select when section report
      foreach ent:sensors setting(v,k)

      pre {
        report_id = event:attr("report_id")
      }

      // Raise event to get report information from each sensor
      event:send({"eci":ent:sensors{[k,"wellKnown_Tx"]},
      "domain":"section", "name":"report_results",
      "attrs":{
        "wellKnown_Tx":subs:wellKnown_Rx(){"id"},
        "Rx_role":"sensor", "Tx_role":"sensor_manager",
        "name":"", "channel_type":"subscription",
        "section_id":k,
        "report_id": report_id,
        "num_sensors": getNumSenesors()
      }
      })

      fired {
        raise section event "upload_report" on final
      }
    }

    rule sensors_receive_report {
      select when section receive_report

        always {
          ent:reports{event:attr("report_id")} := {
            "temperature_sensors" : event:attr("num_sensors"),
                    "responding" : (ent:reports{event:attr("report_id")}{"responding"}.isnull() || ent:reports{event:attr("report_id")}{"responding"} == "") => 1 | ent:reports{event:attr("report_id")}{"responding"} + 1,
                    "temperatures" : ent:reports{event:attr("report_id")}{"temperatures"}.isnull() || (ent:reports{event:attr("report_id")}{"temperatures"} == "") => [event:attr("temperature")] | ent:reports{event:attr("report_id")}{"temperatures"}.append(event:attr("temperature"))
        }
      }
    }

    rule sensors_upload_report {
      select when section upload_report

      send_directive("Report Uploaded!")
    }

    rule clear_report {
      select when section clear_report
