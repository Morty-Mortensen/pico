ruleset wovyn_base {
    ...
    rule set_temperature_threshold {
        select when wovyn set_temperature_threshold

        pre {
            threshold_temp = event:attrs{"threshold"}.klog("New Threshold Temperature: ")
        }

        if not threshold_temp.isnull() && threshold_temp != "" then noop()

        fired {
            ent:threshold_temp := threshold_temp

            raise gossip event "threshold"
        }
    }
